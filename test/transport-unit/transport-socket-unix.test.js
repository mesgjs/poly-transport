...
import { makeLoopbackUnixPair, makeUnixSocketTransportPair } from '../transport-unix-helpers.js';

// ─── Constructor Tests ────────────────────────────────────────────────────────

Deno.test('SocketTransport (Unix) - constructor creates transport in STATE_CREATED', async () => {
...
Deno.test('SocketTransport (Unix) - extends PipeTransport, ByteTransport, and Transport', async () => {
	const { serverConn, clientConn } = await makeLoopbackUnixPair();
	try {
		const transport = new SocketTransport({ conn: serverConn });
...
Deno.test('SocketTransport (Unix) - needsEncodedText returns true', async () => {
	const { serverConn, clientConn } = await makeLoopbackUnixPair();
	try {
		const transport = new SocketTransport({ conn: serverConn });
...
Deno.test('SocketTransport (Unix) - has transport id and logger', async () => {
	const { serverConn, clientConn } = await makeLoopbackUnixPair();
	try {
		const transport = new SocketTransport({ conn: serverConn });
...
Deno.test('SocketTransport (Unix) - write and read a message', async () => {
	...
	await channelA.write(0, 'Hello, Unix socket!');

	const message = await channelB.read({ decode: true });
	assertExists(message);
	assertEquals(message.text, 'Hello, Unix socket!');
	message.done();
...
Deno.test('SocketTransport (Unix) - connection close triggers disconnect', async () => {
	const { serverConn, clientConn } = await makeLoopbackUnixPair();
