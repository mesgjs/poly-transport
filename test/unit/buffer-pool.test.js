...
Deno.test('BufferPool - acquire buffer from pool', () => {
...
Deno.test('BufferPool - acquire selects appropriate size class', () => {
	const pool = new BufferPool();
...
Deno.test('BufferPool - acquire maintains low water mark', () => {
...
Deno.test('BufferPool - release buffer to pool', () => {
	const pool = new BufferPool({ lowWaterMark: 2, highWaterMark: 10 });
...
Deno.test('BufferPool - release zeros buffer', () => {
	const pool = new BufferPool({ lowWaterMark: 0, highWaterMark: 10 });
	...
	// Should be zeroed
	assertEquals(view2[0], 0);
	assertEquals(view2[100], 0);
	assertEquals(view2[1023], 0);
});

// Test: Release enforces high water mark
Deno.test('BufferPool - release enforces high water mark', () => {
...
Deno.test('BufferPool - worker mode request callback', () => {
...
Deno.test('BufferPool - worker mode return callback', () => {
...
Deno.test('BufferPool - multiple acquire/release cycles', () => {
	const pool = new BufferPool({ lowWaterMark: 2, highWaterMark: 10 });
