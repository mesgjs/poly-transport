...
import { OutputRingBuffer, RingBufferReservationError } from '../../src/output-ring-buffer.esm.js';
...
Deno.test('OutputRingBuffer - commit throws on invalid reservation', () => {
	const ring = new OutputRingBuffer(1024);
	const fakeReservation = { length: 10 };
	
	assertThrows(
		() => ring.commit(fakeReservation),
		Error,
		'Invalid reservation'
	);
});
